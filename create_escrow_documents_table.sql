...
CREATE TABLE IF NOT EXISTS public.escrow_documents (
...
CREATE INDEX IF NOT EXISTS idx_escrow_documents_transaction_id ON public.escrow_documents(escrow_transaction_id);
CREATE INDEX IF NOT EXISTS idx_escrow_documents_uploaded_by ON public.escrow_documents(uploaded_by_id);
CREATE INDEX IF NOT EXISTS idx_escrow_documents_created_at ON public.escrow_documents(created_at);
...
DROP POLICY IF EXISTS "Users can view escrow documents they're involved in" ON public.escrow_documents;
...
DROP POLICY IF EXISTS "Users can update documents they uploaded" ON public.escrow_documents;
...
DROP POLICY IF EXISTS "Users can delete documents they uploaded" ON public.escrow_documents;
...
DROP POLICY IF EXISTS "Users can download escrow document files" ON storage.objects;
...
-- Policy for INSERT (Upload) - Users can upload files for escrow transactions
...
DROP POLICY IF EXISTS "Users can update escrow document files they uploaded" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can delete escrow document files they uploaded" ON storage.objects;
...
DROP TRIGGER IF EXISTS update_escrow_documents_updated_at ON public.escrow_documents;
